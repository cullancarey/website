...
resource "aws_route53_zone" "wwwcullancarey" {
  name = "${var.sub_domain}"
  comment       = "Managed by Terraform"
  force_destroy = false
}

resource "aws_route53_record" "ns_wwwcullancarey" {
  name            = "${var.sub_domain}"
  ttl             = 172800
  type            = "NS"
  zone_id         = aws_route53_zone.wwwcullancarey.zone_id

  records = [
    aws_route53_zone.wwwcullancarey.name_servers[0],
    aws_route53_zone.wwwcullancarey.name_servers[1],
    aws_route53_zone.wwwcullancarey.name_servers[2],
    aws_route53_zone.wwwcullancarey.name_servers[3],
  ]
}

resource "aws_route53_record" "soa_wwwcullancarey" {
  name            = "${var.sub_domain}"
  ttl             = 900
  type            = "SOA"
  zone_id         = aws_route53_zone.wwwcullancarey.zone_id
  records = [
    "${trim(aws_route53_zone.wwwcullancarey.name_servers[3], ".")}. awsdns-hostmaster.amazon.com. 1 7200 900 1209600 86400"
  ]
}